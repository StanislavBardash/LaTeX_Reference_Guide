<template>
    <header style="text-align: center;"><h1>Search Results</h1></header>
    <div id="app">
      <div class="container10">
        <div class = "searchnav">
          <strong>Search</strong>
          <!-- <label for="name-search">Name:</label>
          <input type="text" id="name-search" v-model="namSearch"> -->
                    <label for="category-search">Select category:</label>
          <select class="category-search" id="category-search" v-model="catSearch">
            <option v-for="(category,i) in categories1" :key="i" :value="category.name">
                {{ category.name }}
            </option>
          </select>
          <button class="btn btn-primary" @click="search(catSearch)">
...
<script>
    ...
    export default {
        name: `search`, // Имя шаблона
        props: ['categorySearchQuery'],
    ...
    displayedCommands() {
      ...
      return this.commands.slice(start, end);
        ...
        methods: { // методы компонента
            ...
            getCommands() {
                http
                    .get(`/findCommandsWithSearch/categorySearchQuery=${this.categorySearchQuery}`) // обращаемся к серверу для получения списка пользователей
                    .then(response => { // запрос выполнен успешно
            ...
            async search(catSearch) {
              var query = {
                categorySearchQuery: catSearch
              };
              console.log(query);
              this.$router.push({ name: "search", query }).then(() => { this.$router.go() });
    ...
    prevPage() {
      ...
      if (this.currentPage > 1) {
        ...
        mounted() { // загружаем данные пользователей. Обработчик mounted() вызывается после монтирования экземпляра шаблона
            this.catSearch = this.categorySearchQuery;
            this.getCommands();
            this.getCategories();
